...
public class BookTest {
	...
	public static void main(String[] args) {
		...
		books[0] = new Book("플러터UI실전", "김근호"); // 객체 new로 생성
		books[1] = new Book("무궁화꽃이피었습니다", "김진명");
		books[2] = new Book("흐르는강물처럼", "파울로코엘료");
		books[3] = new Book("리딩으로리드하라", "이지성");
		books[4] = new Book("사피엔스", "유발하라리");
		books[9] = new Book("홍길동", "허균");

		// 배열은 반복문과 함께 많이 활용된다.
		// 배열에 크기와 요소의 크기는 꼭 동일한것은 아니다.

		// books[3] --> Book
		// System.out.println(books[3].getTitle());
		// System.out.println(books[3].getAuthor());

		// System.out.println(books[5].getTitle); <-- nnull pointer 에러

		for (int i = 0; i < books.length; i++) { // books[0]. // books[1]. //
			// books[2]. // books[5]. 가리키는 주소가 없음
			// 방어적 코드를 작성해주어야 한다.
			if(books[i] != null) { // 널이 아니라면 출력해라!
